...
newdata <- PlasmodeSim::simulateSurvivaltimes( #(plpModel, plpData, number, population, populationSettings)
  plpModel = plpModel,
  plpData = plpData ,
  number = 10,
...
TrainingSet <- PlasmodeSim::MakeTraingSet(
  ...
  outcomeId=3
)          # this sometimes fials then i run again and it works fine?

fitcensor <- PlasmodeSim::fitCensoring(
...
unfittedcensor<- list(censorModel =  fitcensor$censorModel,
                      outcomesModel = unfittedmodel) #is this okey or should we have a functin that does this. 

NewOutcomes <- PlasmodeSim::simulateSurvivaltimesWithCensoring(
  fitCensor = unfittedcensor,
  plpData = plpData,
  populationSettings = populationSettings,
  numberToSimulate = 20
)
...
````{R,message= F  }
#AdjustBaselineSurvival(plpModel,3592 ,0.87, expLp(plpData,plpModel), intervalSolution= c(-100,100))
